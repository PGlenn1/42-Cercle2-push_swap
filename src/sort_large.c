...
void	get_elem(t_stack *stack, t_limits *limits, int target_index)
{
	if (stack->first->index > limits->limit_c)
		stack->operator= find_index(stack, target_index, limits->median_d,
				limits->limit_c);
	else if (stack->first->index > limits->limit_b)
		stack->operator= find_index(stack, target_index, limits->median_c,
				limits->limit_b);
	else if (stack->first->index > limits->limit_a)
		stack->operator= find_index(stack, target_index, limits->median_b,
				limits->limit_a);
	else
		stack->operator= find_index(stack, target_index, limits->median_a, 0);
}

int	get_current_segment(t_stack *stack, t_limits *limits)
{
	if (stack->first->index > limits->median_d)
		return (limits->median_d);
	else if (stack->first->index > limits->limit_c)
		return (limits->limit_c);
	else if (stack->first->index > limits->median_c)
		return (limits->median_c);
	else if (stack->first->index > limits->limit_b)
		return (limits->limit_b);
	else if (stack->first->index > limits->median_b)
		return (limits->median_b);
	else if (stack->first->index > limits->limit_a)
		return (limits->limit_a);
	else if (stack->first->index > limits->median_a)
...
void	sort_large_numbers(t_root *root)
{
	...
	while (!final_is_sorted(root))
	{
		...
		if (phase_b)
		{
			if (root->stack_b->size == 0)
				root->stack_a->operator= ROT;
			else
